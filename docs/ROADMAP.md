...
| 013 | Recall default filter + provenance + outdated | Proposed | ⏳ **未着手**                                          |
...
- **013 → 014 → 012**: 013 は recall 失敗の即時解消 (1 日工数の G が特大効果)、014 は架構変更で重い、012 は 014 後に必要性を再評価して着手判断
- **013 と 014 は並走可能**: 別ファイルを編集、interface 改善 (013) vs 階層追加 (014) なので衝突しない
