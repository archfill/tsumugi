...
### 本番評価を受けた改訂 (local、本番反映待ち)

- promotion は既存 fact backlog を先に drain し、retry 待ちの間は observation seed と capture window 作成を止める
- provider endpoint + credential 単位の circuit breaker (5〜30 分、half-open 単一 probe) を追加
- lease / fencing の `attempt_count` と item quarantine 用 `failure_count` を分離し、provider 障害を item failure に数えない
- observation の fact 抽出失敗にも durable backoff / quarantine を追加する
- time-update / synthesize / promotion の provider cooldown を partial run として記録し、retry / circuit metrics と Admin UI で観測可能にする
