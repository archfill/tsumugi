...
過去文脈が必要なら推測する前に `search_memory` を呼ぶ。
通常は project-scoped search に任せ、明示的に cross-project / session-only
recall が必要な時だけ `filter.project_tag: null` を渡す。
memory hit の `provenance` を見て由来を確認する。
古い memory を見つけたら `mark_memory_outdated` を呼ぶ。
```
...
3. 検索したい過去 memory があれば project-scoped `search_memory` を呼んでください
