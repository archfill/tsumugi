...
| `GET /api/admin/overview`               | Three-layer 集計、queue、current attention、run history、schedule |
...
| `POST /api/admin/operations/issues/:kind/:id/retry` | window / fact / observation の明示的な再投入 |
...
初回改訂では retry / unquarantine / mark outdated の mutation を追加せず、既存 Memory edit /
archive のみ維持した。2026-07-14 の本番評価で、quarantine 後の回復にDB直接操作が必要になる
ことを確認したため、Operations issue のうち window / fact / observation に限定して確認付きの
retry / restore mutation を追加する。raw capture は response に返さず、quarantined window の
入力復元は retention 中の capture をserver内で再構成する。

`attention_count` はcurrent issue一覧と同じread model / filter scopeで、現在retry / reviewが
必要なquarantine、期限到来済みdefer、stale lease、outdatedのみを数える。過去のfailed /
partial dreaming runはproject / source帰属を持たないglobalな `history_issue_count` と
`GET /api/dreaming/runs` に分離し、回収済みの履歴を現在障害として表示しない。
...
- Operations は job / run / issue の状態確認を優先し、回復mutationは対象ID・状態を検証して
  明示確認後に実行する
