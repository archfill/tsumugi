...
| Hook               | 追加役割                                                                    |
| ------------------ | --------------------------------------------------------------------------- |
| `PostToolUse(*)`   | **新規**: tool 結果を `captures` テーブルに直接 insert (LLM 呼出無し)       |
| `Stop`             | **新規**: session 終了で transcript 全体を `captures` に保存 + 昇格 trigger |
| `SessionStart`     | (既存維持) memory + rubric inject                                           |
| `UserPromptSubmit` | (既存維持) resume / error pattern 検出時 inject                             |
| `PreToolUse(Read)` | (既存維持) file 関連 memory inject                                          |

#### capture 範囲 (PostToolUse)

**全 tool 捕捉**。Bash / Read / Edit / Write / MCP tool すべて。
volume の懸念は Layer 1 が隔離されており Layer 2 純度に影響しないこと、
30 日 TTL で自動 sweep されることでカバー。

ADR-011 で禁止された「**raw を Layer 1 (旧定義) に投げる**」は `observations` テーブルに
直接 insert することを指していた。本 ADR では新 `captures` テーブルにのみ insert
されるので、ADR-011 の精神 (= observation 純度) は維持される。
