# Tsumugi server multi-stage Dockerfile
#
# Stages:
#   1. builder : pnpm workspace の server + shared を install + build
#   2. runtime : 必要な node_modules + dist のみコピー
#
# UI は apps/ui/Dockerfile で nginx image として独立 build。
# このイメージには UI dist は含まない。
#
...
# server + shared の依存だけインストール (UI を含まない)
# native build 許可は pnpm-workspace.yaml の allowBuilds が効く
RUN pnpm install --frozen-lockfile --filter @tsumugi/server...

# ソースコピー
COPY packages/shared ./packages/shared
COPY apps/server ./apps/server

# server (と shared) を build
RUN pnpm --filter @tsumugi/server... -r build
